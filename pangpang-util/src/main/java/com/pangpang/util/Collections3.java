...
public class Collections3 {
    ...
    /**
     * 类似extractToMap，区别在于MAP的Value是一个List
     *
     * @param collection 来源集合.
     * @param keyPropertyName 要提取为Map中的Key值的属性名.
     * @param valuePropertyName 要提取为Map中的Value值的属性名.
     * @return
     */
    public static Map extractToMapList(final Collection collection, final String keyPropertyName,
            final String valuePropertyName) {
        Map<Object, List> map = new LinkedHashMap<Object, List>();

        try {
            for (Object obj : collection) {
                Object k = PropertyUtils.getProperty(obj, keyPropertyName);
                Object v = PropertyUtils.getProperty(obj, valuePropertyName);
                if (map.get(k) == null) {
